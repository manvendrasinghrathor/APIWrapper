...
//  animateLabel
//
//  Created by Shashi kumar on 27/10/18.
//  Copyright © 2018 Shashi kumar. All rights reserved.
//
...
class AnimatedLabel: UILabel {
    ...
    fileprivate var currentValue: Float {
        ...
        return startingValue + (update(ttt: Float(progress / totalTime)) * (destinationValue - startingValue))
    }
    ...
    fileprivate func update(ttt: Float) -> Float {
        var ttt = ttt
        switch countingMethod {
        case .linear:
            return ttt
        case .easeIn:
            return powf(ttt, rate)
        case .easeInOut:
            var sign: Float = 1
            if Int(rate) % 2 == 0 { sign = -1 }
            ttt *= 2
            return ttt < 1 ? 0.5 * powf(ttt, rate) : (sign*0.5) * (powf(ttt-2, rate) + sign*2)
        case .easeOut:
            return 1.0-powf((1.0-ttt), rate);
        }
    }
